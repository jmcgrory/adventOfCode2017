  ...
  for(var i = 0; i < rows.length; i++){
    ...
    for(var ii = 0; ii < row.length; ii++){
      ...
      if( high === undefined || int > high ){ high = int }
      if( low === undefined || int < low || int == 0 ){ low = int }

    }

    difference = ( high - low );

    console.log( 'high '+high, 'low '+low, 'diff '+difference );

    tally.push(difference);
  }
  ...
  if (tally.length) {
    console.log(tally);
    console.log(tally.length*5);
    total = tally.reduce(getSum);
  }


  // Write Total to Body
  var insert = document.getElementById("target");
  insert.innerHTML += total;