  ...
  for(var i = 0; i < rows.length; i++){
    ...
    for(var ii = 0; ii < row.length; ii++){
      ...
      // Divisible loop
      for(var iii = ii; iii < row.length; iii++){

        var high = ( int > row[iii] ) ? int : row[iii];
        var low = ( int > row[iii] ) ? row[iii] : int;
        var divi = high / low;

        if( divi%1==0 && high && low && high !== low ){
          whole.push(divi);
        }
      }
    }
